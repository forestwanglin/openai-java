...
import static xyz.felh.openai.jtokkit.TokenEncoder.MAX_RANK;
...
final class TokenEncoderLarge {
    ...
    static int calculateTokensLarge(TokenEncoder tokenEncoder, int maxTokenCount, boolean keepEncodings, List<Integer> out, ByteArrayWrapper match, int length) {
        ...
        while (validRanks > 0) {
            ...
            int firstIndex;
            for (Map.Entry<Integer, RankNode> entry = minNodes.firstEntry(); entry != null; entry = minNodes.ceilingEntry(firstIndex)) {
